...
START_TEST(test_s21_sum_matrix_valid) {
  ...
  ck_assert_int_eq(s21_sum_matrix(&mat1, &mat2, &result), S21_OK);
...
START_TEST(test_s21_sum_matrix_diff_sizes) {
  matrix_t mat1, mat2, result;
  s21_create_matrix(2, 2, &mat1);
  s21_create_matrix(3, 2, &mat2);

  ck_assert_int_eq(s21_sum_matrix(&mat1, &mat2, &result), S21_CALC_ERROR);
...
START_TEST(test_s21_sum_matrix_null_pointers) {
  ...
  ck_assert_int_eq(s21_sum_matrix(NULL, &mat1, &result), S21_ERROR);
  ck_assert_int_eq(s21_sum_matrix(&mat1, NULL, &result), S21_ERROR);
  ck_assert_int_eq(s21_sum_matrix(&mat1, &mat1, NULL), S21_ERROR);
...
Suite *s21_sum_matrix_suite(void) {
  ...
  s = suite_create("\033[45ms21_sum_matrix\033[0m");
  tc_core = tcase_create("Core");

  tcase_add_test(tc_core, test_s21_sum_matrix_valid);
  tcase_add_test(tc_core, test_s21_sum_matrix_diff_sizes);
